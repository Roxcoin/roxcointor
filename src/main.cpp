...
uint256 const hashGenesisBlock("0x00000483bb753af473061cd4bc0edd5c5dc54e6b2cb18c42dfaeec4079dbc42c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Roxcoin: starting difficulty is 1 / 2^12
...
{
    // Roxcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
	if (nHeight == 1) { return 4501000 * COIN; } // swap ROX x11
	int64 nSubsidy = 1000 * COIN;
	int64 nSubsidyHalvingInterval = 720;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Roxcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Roxcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
       const char* pszTimestamp = "Blockchain.info Block #310148 2014-07-10 22:59:56";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xcad6cd737106ba5519470846b755b1657d7207e28b7f81e32f711172cedf10d2"));
...
unsigned char pchMessageStart[4] = { 0xa1, 0xb9, 0xaa, 0x7f }; // Roxcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("RoxcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RoxcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static RoxcoinMiner(CWallet *pwallet)
{
    printf("RoxcoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running RoxcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("RoxcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RoxcoinMiner, pwallet));
}
